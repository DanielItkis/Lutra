...
// 
// In a second step, the user has to define the line that is marked by the clip on one film
// orientation relatively to the laserbeam is calculated from that
// On each image, the dark marks left by the holes in the film have to be selected
// to calculate the relative angle of the films
// all films are rotated and shifted, so that they can be stacked
...
public class Cut_Film implements PlugIn{
  ...
  public void cutting(int nx, int ny, String directory, String[] titles, String titletiff){
    ...
    Roi[] Rois; // Array to get Rois from ROI manager
	Rectangle[][] r = new Rectangle[nx][ny]; // Array of Rectangles to save ROIs for cutting
    Toolbar tool = new Toolbar(); // can access the ImageJ toolbar
    WaitForUserDialog samplerois,showall;
    int x1,y1,width,height,xdist,ydist=0;
    String bdepth=new String();
	int nfilm; // after cutting, number of films is counted (may change because user can delete ROIs)
    ...
    showall.show();
	// rectman.runCommand("Select All"); // doesn't seem to work yet, maybe not implemented?
    
  	Rois=rectman.getRoisAsArray(); // ROIs from the manager are returned as array
	
	nfilm=rectman.getCount();
	for (int n=1; n<=nfilm;n++){ // changed ROIs are written into Array
    ...
    // This section copies the selected ROIs into new images, each containing only one film
		nfilm=rectman.getCount();
  ...
  public void rotate(int nfilm, String directory, String[] titles, String titletif){
    ...
    double[][] results = new double[nfilm][4]; // Array containg coordinates of centres of mass of ROIs around the holes
    Rectangle[] rrot = new Rectangle[2]; // Array of Rectangles to save ROIs: 2 ROIs for every film
    ...
    if (nfilm>1) IJ.selectWindow(titletif+"film"+String.format("%02d",nfilm)+".tif");
    implus=WindowManager.getCurrentImage();
    
    // all films are rotated so that the clip is vertical (-> absolute position)
    ...
    // For each image, 2 ROIs have to be selected to rotate them relatively to each other
  //  int n=0;
    for (int n=0; n<nfilm; n++){
		...
		for (int i=0; i<2; i++){
			...
			wfud=new WaitForUserDialog("Select hole "+(i+1)+" in film "+(n+1)+" using the wand tool.");
			wfud.show();
			
			stats=implus.getStatistics(64); // 64 = Center of Mass (static integer value)
			wfud.close();
			results[n][2*i]=stats.xCenterOfMass; // center of mass of each hole is used as reference point for rotation
			results[n][2*i+1]=stats.yCenterOfMass;
		} // for
		...
		// angle between current image and first image (reference) is calculated using the direct product
		// angle for first image equals zero (ore if only one image)
		scalprod = (results[n][2]-results[n][0])*(results[0][2]-results[0][0])+(results[n][3]-results[n][1])*(results[0][3]-results[0][1]);
		absval = Math.sqrt((Math.pow((results[n][2]-results[n][0]),2)+Math.pow((results[n][3]-results[n][1]),2))*(Math.pow((results[0][2]-results[0][0]),2)+Math.pow((results[0][3]-results[0][1]),2)));
		angle = Math.acos((scalprod)/(absval))/(2*Math.PI)*360; 
		IJ.run("Arbitrarily...", "angle="+(angle+rotall)); // rotation relatively to each other and relatively to the beam position
		
		// cut all images to the same size and center on hole in the middle
		implus.setRoi((int)(results[n][0]-results[0][0]+0.05*ip.getWidth()), (int)(results[n][1]-results[0][1]+0.05*ip.getHeight()), (int)(0.9*ip.getWidth()), (int)(0.9*ip.getHeight()));
		IJ.run("Crop");
