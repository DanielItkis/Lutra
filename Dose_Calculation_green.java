...
public class Dose_Calculation_green implements PlugIn{
	...
	public void CalculationOfDose(String directory, int i, double p1, double p2, double p3, double bckg, double unexp, double maxdos, String title){
		...
		IJ.selectWindow(title + " (red)"); // Green channel is selected as active window
	...
	public void run(String arg) {
		IJ.register( Dose_Calculation_green .class);
		...
		gd = new GenericDialog("Enter Parameters GREEN Channel", IJ.getInstance()); 
		gd.addMessage("Please insert your parameters for calibration");
		gd.addNumericField("p1", 10.44, 3); // Suggestions for parameters
		gd.addNumericField("p2", 35.44, 3);
		gd.addNumericField("p3", 1.4, 3);
		...
		gd.addNumericField("Maximum dose film can detect", 50, 0);
