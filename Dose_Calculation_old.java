...
public class Dose_Calculation implements PlugIn{
	...
	public void run(String arg) {
		...
		String[] editp={"default", "customized"}; // Scroll-down for editing p-values
		...
		customgd = new GenericDialog("Customize parameters", IJ.getInstance()); //Dialogue that only appears if p-values set to "customized"
		customgd.addMessage("Customize parameters"); 
		customgd.addNumericField("Red (low dose) p1", 15, 5); // Suggestions for parameters
		customgd.addNumericField("Red (low dose) p2", 15, 5);
		customgd.addNumericField("Red(low dose) p3", 5, 3);
		customgd.addNumericField("Green (medium dose) p1", 5, 3);
        customgd.addNumericField("Green (medium dose) p2", 5, 3);
        customgd.addNumericField("Green (medium dose) p3", 5, 3);
