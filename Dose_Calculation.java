...
public class Dose_Calculation implements PlugIn{
	...
	public void CalculationOfDoseblue(String directory, int i, double p1, double p2, double p3, double bckg, double unexp, double maxdos, String title){
		...
		// From now on, only Blue channel is used
	...
	public void run(String arg) {
		IJ.register( Dose_Calculation .class);
		...
		gd = new GenericDialog("Parameters", IJ.getInstance()); 
		gd.addChoice("Choose channel", editp, "red");
		...
		gd.addChoice("Save as B/W?", savemodes, "Don't save");
		gd.showDialog();
		if (gd.wasCanceled()) return; // User may cancel Plugin

		editop = gd.getNextChoice();
		bckg = gd.getNextNumber();
		unexp =  gd.getNextNumber();
		maxdos =  gd.getNextNumber();
		savemode=gd.getNextChoice();

		if (editop.equals("all")==true){
		customgd = new GenericDialog("Customize parameters all", IJ.getInstance()); //Dialogue that only appears if p-values set to "customized"
		customgd.addMessage("Customize parameters all"); 
		//default values measured by Johannes Konrad (j.konrad@hzdr.de), added 12092016
        ...
        customgd.addNumericField("Blue (high dose) p1", 5, 3);
        customgd.addNumericField("Blue (high dose) p2", 5, 3);
        customgd.addNumericField("Blue (high dose) p3", 5, 3);
		customgd.showDialog();
		...
		else if (editop.equals("blue")==true){
			customgd = new GenericDialog("Customize parameters blue", IJ.getInstance()); //Dialogue that only appears if p-values set to "customized"
		...
		while (i <= numberoffiles) { // go through each of the opened images 
			IJ.selectWindow(titles[i-1]);
			//IJ.run("Flip Horizontally"); // images are flipped so the direction the user looks at them equals the direction where the beam came from
			CalculationOfDoseblue(directory,numberoffiles,p1,p2,p3,bckg,unexp,maxdos,titles[i-1]); // Calculates dose for every opened image
			i++;
		} // while
		...
		if (savemode.equals("Choose directory")==true) directory = IJ.getDirectory("Directory to save to..."); //case savemode=Choose directory
			...
			customgd = new GenericDialog("Customize parameters green", IJ.getInstance()); //Dialogue that only appears if p-values set to "customized"
		...
		if (savemode.equals("Choose directory")==true) directory = IJ.getDirectory("Directory to save to..."); //case savemode=Choose directory
	   		...
	   		customgd = new GenericDialog("Customize parameters red", IJ.getInstance()); //Dialogue that only appears if p-values set to "customized"
