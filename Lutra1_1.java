...
public class Lutra1_1 implements PlugIn{
...
// Calculates the dose from the red values in the image
	...
	public void run(String arg) {
		...
		p2 =  customgd.getNextNumber();
		p3 =  customgd.getNextNumber();
		}else{p1 = 0;p2 = 0; p3=0; IJ.error("Invalid Color Channel. Please restart plugin.");
}
		...
		while (i <= numberoffiles) { // go through each of the opened images 
			...
			CalculationOfDosered(directory,numberoffiles,p1,p2,p3,bckg,unexp,maxdos,titles[0]); // Calculates dose for every opened image}
		}
		else if (imagename == titles[1]){IJ.selectWindow(titles[1]);
			CalculationOfDosered(directory,numberoffiles,p1,p2,p3,bckg,unexp,maxdos,titles[1]); // Calculates dose for every opened image}
		}	
		else if (imagename == titles[2]){IJ.selectWindow(titles[2]);
			CalculationOfDosered(directory,numberoffiles,p1,p2,p3,bckg,unexp,maxdos,titles[2]); // Calculates dose for every opened image}
		}	
		else if (imagename == titles[3]){IJ.selectWindow(titles[3]);
			CalculationOfDosered(directory,numberoffiles,p1,p2,p3,bckg,unexp,maxdos,titles[3]); // Calculates dose for every opened image}
		}	
